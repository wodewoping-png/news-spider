...
正文默认以 500 个字符作为完整性告警阈值，可用 `--min-content-chars` 调整。重跑同一日期时，
程序会重新访问已有但低于阈值的短正文；只有新提取结果更长时才原位更新 JSONL，避免重复
文章或把已有正文覆盖成更短的摘要。
...
9. 正文提取会比较页面正文节点、列表/表格/引用内容与 JSON-LD `articleBody`，保留质量得分最高的完整正文；重跑时可升级已有短正文。
10. 输出 `data/articles.jsonl`、带名义运行日期的 CSV 和 `logs/channel-health.json`；日更渠道零产出或正文少于 500 字会明确标记为 zero/degraded，周刊、月刊当天无更新则标记为 idle。
...
- `logs/channel-operations.csv`：每个渠道一行的当前运维状态表，包含最近成功时间、连续异常次数、正文最小/中位/最大字符数、短正文数、待补日期数和下一步动作。
...
- `logs/channel-daily-stats.csv`：每个渠道每天一行，包含收集量、唯一文章数、正文可用率、正文最小/中位/最大字符数、短正文数、前次收集量、7 日中位数、变化倍数、连续为零天数和异常原因。
...
- 正文少于 500 字计为短正文；正文可用率低于 70%，或重复文章占比达到 20%：标记质量异常。
